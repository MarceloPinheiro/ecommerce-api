...
RSpec.describe "Admin::V1::SystemRequirements as :admin", type: :request do
    ...
    context "PATCH /system_requirements/:id" do
        ...
        context "with valid params" do
            ...
            let(:system_requirement_params) { { system_requirement: { name: new_name, memory: new_memory } }.to_json }

            it 'updates System Requirements' do
    ...
    context "DELETE /system_requirements/:id" do
        ...
        it 'does not remove unassociated games' do
            games = create_list(:game, 3)
            delete url, headers: auth_header(user)
            present_games_ids = games.map(&:id)
            expected_games = Game.where(id: present_games_ids)
            expect(expected_games.ids).to contain_exactly(*present_games_ids)
        end
    end
